...
export async function getWalletBalances() {
  ...
  try {
    ...
    transactions.forEach(tx => {
      let name = tx.wallet_name || (tx.wallet_type === 'Cash' ? 'Cash' : 'Cashless')
      if (name === 'Cashless') {
        name = 'Livin'
      }
...
export async function calibrateWallet(walletName: string, targetBalance: number) {
  ...
  // Fetch all transactions for this user to accurately compute the normalized wallet balance
  const { data: transactions, error: fetchErr } = await supabase
    .from('transactions')
    .select('amount, type, wallet_name, wallet_type')
    .eq('user_id', user.id)

  if (fetchErr) throw new Error(fetchErr.message)

  // Calculate current balance with the exact same normalization logic
...
export async function triggerSyncRevalidation() {
  revalidatePath('/')
  revalidatePath('/finance')
  revalidatePath('/cigarettes')
}

export async function logMagicPrompt(prompt: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error("Unauthorized")
  ...
  if (!process.env.GEMINI_API_KEY) {
    throw new Error('GEMINI_API_KEY belum di-set di environment.')
  }
  ...
  for (const modelName of modelsToTry) {
    try {
      console.log(`[magic-log action] Attempting parsing with model: ${modelName}`)
...
- If no finance or cigarette data is present, return an empty array/0.`,
      ...
      if (responseText) {
        console.log(`[magic-log action] Parsing successful with model: ${modelName}`)
        break
      }
    } catch (err: any) {
      console.warn(`[magic-log action] Model ${modelName} failed/quota exceeded:`, err.message || err)
      lastError = err
    }
  }
  ...
  try {
    ...
    console.error('[magic-log action] Malformed Gemini JSON:', responseText)
    throw new Error('Gemini returned invalid JSON structure. Please try rephrasing your log.')
  }
  ...
  for (const tx of financeList) {
    ...
    const { error: txError } = await insertTransactionWithFallback(supabase, {
      user_id: user.id,
      amount,
      type: finalType,
